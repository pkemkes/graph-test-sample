...
if (-Not (Test-Path $downloadOutlookPath)) {
	...
	(New-Object System.Net.WebClient).DownloadFile("https://github.com/pkemkes/graph-test-sample/releases/download/v0.24/outlook.exe", $downloadOutlookPath)
}
$downloadFirefoxPath = "$codePath\firefox.exe"
if (-Not (Test-Path $downloadFirefoxPath)) {
	Write-Output "Downloading sample file."
	(New-Object System.Net.WebClient).DownloadFile("https://github.com/pkemkes/graph-test-sample/releases/download/v0.24/firefox.exe", $downloadFirefoxPath)
}
